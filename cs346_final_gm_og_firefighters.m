...
prob_init_fighter = 0.05; % initial probability fire fighter spawns
    ...
    for row = 2:row_count + 1
        for col = 2:col_count + 1
            ...
            if(grid_point == FIGHTER)
                ...
                [fire_rows, fire_cols] = ind2sub(extended_grid_size, find(extended_forest == FIRE));
                % Arbitrary distance metric of just subtracting current row and column values
                % from fire_rows/cols and adding the result to use. Lowest value in this list
                % will be the "closest fire"
                distance_list = [fire_rows - row + fire_cols - col];
                ...
                fire_row = fire_rows(closest_fire_idx);
                fire_col = fire_cols(closest_fire_idx);
                ...
                while(moves_left > 0)
                    ...
                    if(fire_row ~= row)
                        new_row = new_row + (1 * sign(fire_row - row));
                    end

                    if(fire_col ~= col)
                        new_col = new_col + (1 * sign(fire_col - col));
                    end
                end
                ...
                forests(new_row, new_col, frame) = FIGHTER_MOVE;
...
wet_dirt_color; wet_grass_color; wet_tree_color; fighter_color; figher_move_color]); % Dirt, Tree, Fire
