...
dt = 1; % timestep
simLength = 12; % length of simulation
...
prob_init_tree = 0.3; % initial probability a cell is a tree
prob_init_grass = 0.8; % initial probability a cell is grass
prob_init_fire = 0.25; % initial probability a tree is on fire
...
% Direction that rain moves
rain_direction = "NW"; % TODO: make this based on wind
...
%     extented_rain_grid(2:end-1, 2:end-1) = rain_grids(:,:,frame-1);
    ...
    for row = 2:row_count + 1
        for col = 2:col_count + 1
            ...
            % Rain movement
            rain_grid_point = rain_grid(row-1, col-1);
            updated_rain_cell = rain_grid_point;
            
            if(mod(frame, cloud_move_const) == 0)
                disp("outside row: " + (row) + " col: " + col);
                disp(frame);
                updated_rain_cell = update_rain(rain_direction, rain_grids, frame, row-1, col-1);
            end            
            ...
            rain_grids(row - 1, col - 1, frame) = updated_rain_cell;
        end
    end
end
...
%%
function updated_rain_cell = update_rain(rain_direction, rain_grids, frame, row, col)
    row_count = 10;
    col_count = 10;
    updated_rain_cell = 1;
    ...
    if rain_direction == "N"
        if (1 < row) && (row < row_count) && (1 < col) && (col<col_count)
                updated_rain_cell = rain_grids(row - 1, col, frame - 1);            
        end

    % Rain cloud moving south
    elseif rain_direction == "S"
        if (1 < row) && (row < row_count) && (1 < col) && (col<col_count)
                updated_rain_cell = rain_grids(row + 1, col, frame - 1);
    ...
    elseif rain_direction == "E"
        if (1 < row) && (row < row_count) && (2 < col) && (col<col_count)
                updated_rain_cell = rain_grids(row, col-1, frame - 1);
            
        end

    % Rain cloud moving west
    elseif rain_direction == "W"
        if (1 < row) && (row < row_count) && (1 < col) && (col<col_count-1)
                disp(col);
                disp(col_count);
                updated_rain_cell = rain_grids(row, col+1, frame - 1);
        end
    ...
    elseif rain_direction == "NE"
        if (2 < row) && (row < row_count) && (2 < col) && (col<col_count)
                updated_rain_cell = rain_grids(row - 1, col - 1, frame - 1);
        end
        

    % Rain cloud moving northwest
    elseif rain_direction == "NW"
        if (2 < row) && (row < row_count) && (1 < col) && (col<col_count-1)
                updated_rain_cell = rain_grids(row-1, col+1, frame - 1);
        end
        
    % Rain cloud moving southeast
    elseif rain_direction == "SE"
        if (1 < row) && (row < row_count-1) && (2 < col) && (col<col_count)
                updated_rain_cell = rain_grids(row+1, col-1, frame - 1);
        end

    % Rain cloud moving southwest
    elseif rain_direction == "SW"
        if (1 < row) && (row < row_count-1) && (1 < col) && (col<col_count-1)
                updated_rain_cell = rain_grids(row+1, col+1, frame - 1);
        end
